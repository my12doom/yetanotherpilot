...
static void LCD_CtrlLinesConfig(void)
{
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);
  	GPIO_SetBits(GPIOB, GPIO_Pin_1);		//取消 LCD RESET。
}
...
static void LCD_FSMCConfig(void)
{
	...
	/* FSMC读速度设置 */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 1;//5;  /* 地址建立时间  */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 5;//5;	   /* 数据建立时间  */
	...
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 7;//3;	   /* 数据建立时间  */
	FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
	FSMC_NORSRAMTimingInitStructure.FSMC_CLKDivision = 0x00;
	FSMC_NORSRAMTimingInitStructure.FSMC_DataLatency = 0x00;
	FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;	/* FSMC 访问模式 */
...
* Description    : 读BMP图片并显示
* Input          : x，y：显示地址  bmp_name：文件名
* Output         : None
* Return         : None.
* Attention		 : None
*******************************************************************************/
void Picshow(int x,int y, const char *bmp_name)					 
{
  ...
  res = f_open(&file, bmp_name, FA_OPEN_EXISTING | FA_READ);//打开图片
  res = f_read(&file, newdata, 0x36, &br);//读取文件头信息
  if(newdata[0]==0x42 && newdata[1]==0x4D)//文件格式标记为BM
  {
  temp  = newdata[18]+(newdata[19]<<8)+(newdata[20]<<16)+(newdata[21]<<24);//图片宽度信息
  temp1 = newdata[22]+(newdata[23]<<8)+(newdata[24]<<16)+(newdata[25]<<24);//图片高度信息
  temp2 = newdata[28]+(newdata[29]<<8);																						//图片位深度信息
		//24位深度图片
		if(temp2==24)
		{
			LCD_SetCursor(0,0); 	
			...
			while(1)
			{
				res = f_read(&file, newdata, 3*1024, &br);//读取3个数据
				if(res||br==0)//错误跳出
				break;
				for(i=0;i<br/3;i++)
				{	/***************画点函数**************/					         
		...
		{	
			...
			while(1)
			{
				res = f_read(&file, newdata, 4*1024, &br);//读取3个数据
				if(res||br==0)   
				break;
				for(i=0;i<br/4;i++)
				{/***************画点函数**************/
		...
		if(temp2==16)
		{	
			res = f_read(&file, newdata, 0x10, &br);//读取文件头信息
			LCD_SetCursor(0,0); 	
			LCD_WriteIndex(0x0022);
			while(1)
			{
				res = f_read(&file, newdata, 2*1024, &br);//读取3个数据
				if(res||br==0)   
				break;
				for(i=0;i<br/2;i++)
				{/***************画点函数**************/
...
{
  int i=0;
  int a=0,b=0;//a为横坐标，b为纵坐标
  FIL file;//文件结构体定义
  UINT br;
  FRESULT res;
  uint8_t newtable[2];
  uint8_t table[2]={0,0};				 
  res = f_open(&file,txt_name, FA_OPEN_EXISTING | FA_READ);//打开文件        
  if(res!=FR_OK)//打开失败串口发送失败信息，可省略
  printf("error");        
  while(1)	
  {
		res = f_read(&file, newdata,1024, &br);//读取1024个数据
		if(res || br == 0)//失败，退出
		break;  
		kk:	while(i<br)
		{
			while(newdata[i]==13&&newdata[i+1]==10)	  //判断回车符
			{
				b=b+16;//纵坐标换行
				a=0;//横坐标归0
				i=i+2;//跳过回车符2个
			}
			newtable[0]=newdata[i];//每两字节缓存
			newtable[1]=newdata[i+1];
			if(newdata[i]>128)//中文显示
			{
					if(i==1023)//出现半个字节
					{
					table[0]=newdata[i];//扇区末尾半字节存储
					break;	
				    }
					if(!table[0])
					{
						PutChinese(a, b, newtable,White ,Black);//正常显示
						i=i+2;
						a=a+16;																						 //横坐标加16
					}	
						else//半字节处理
						{	
							table[1]=newdata[i];				 //另外半字节
							newtable[0]=table[0];
							newtable[1]=table[1];
					...
					if(a>225)									 //横坐标超出范围，换行，225+16=241>240
					{
						a=0;
						b=b+16;
					}
				...
				else					 //英文字符显示
				{
					...
					i=i+1;
					a=a+8;
					if(a>233)									//横坐标超出范围，换行，233+8=241>240
					{
					a=0;
					b=b+16;
					}
				}
			while(b>305)//纵坐标超出范围，换页
			{
			//	if(down==0)//判断"下"按键
			//	{
			//		b=0;
