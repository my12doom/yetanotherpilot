...
const int page_count = buffer_size/page_size;
const int space_size = (page_count-1)*page_size;
	...
	while (pos>0)		// >0 instead of >=0, the meta data is not returned
	{
		...
		if (header.key_size == keysize && header.delete_tag == fresh_value)
		{
			...
			if (memcmp(key_read, key, keysize) == 0)
			{
				...
				return space_raw_write(pos + sizeof(header) - sizeof(header.delete_tag), &delete_tag, sizeof(header.delete_tag));
			}
		}
...
int space_write(const void *key, int keysize, const void *data, int num_to_write, int *num_written)
{
	...
	int result = space_raw_write(write_pointer, &header, sizeof(header) - sizeof(header.delete_tag));		// don't write the delete tag
	write_pointer += sizeof(header);
	if (result < 0)
		return result;

	result = space_raw_write(write_pointer, key, keysize);
	write_pointer += keysize4;
	if (result < 0)
		return result;

	result = space_raw_write(write_pointer, data, num_to_write);
	write_pointer += datasize4;
	if (result < 0)
		return result;

	result = space_raw_write(write_pointer, &footer, sizeof(footer));
	...
	while((search_pos = find_next_entry(search_pos)) >= 0)
	{
		...
		if (header.delete_tag == fresh_value)
		{
			...
			if (start_page>page_erased)
			{
				...
				space_raw_erase(page_erased*page_size);
			}
			...
			int res = space_raw_write(write_pointer, buf, sizeof(header)-sizeof(header.delete_tag));
			if (res<0)
				return -5;

			write_pointer += sizeof(header);
			size = space_raw_write(write_pointer, buf+sizeof(header), size-sizeof(header));
			if (size<0)
				return -5;
