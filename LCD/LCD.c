...
static void LCD_CtrlLinesConfig(void)
{
	...
	/* Set PD.07(NE1),  as alternate function push pull - NE1(LCD /CS) */
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);
  	GPIO_SetBits(GPIOB, GPIO_Pin_1);		//È¡Ïû LCD RESET¡£
}
...
static void LCD_FSMCConfig(void)
{
	...
	/* FSMC¶ÁËÙ¶ÈÉèÖÃ */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 1;//5;  /* µØÖ·½¨Á¢Ê±¼ä  */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 5;//5;	   /* Êý¾Ý½¨Á¢Ê±¼ä  */
	...
	/* FSMCÐ´ËÙ¶ÈÉèÖÃ */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 5;//1;   /* µØÖ·½¨Á¢Ê±¼ä  */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 7;//3;	   /* Êý¾Ý½¨Á¢Ê±¼ä  */
	FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
	FSMC_NORSRAMTimingInitStructure.FSMC_CLKDivision = 0x00;
	FSMC_NORSRAMTimingInitStructure.FSMC_DataLatency = 0x00;
	FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;	/* FSMC ·ÃÎÊÄ£Ê½ */
...
/*******************************************************************************
* Function Name  : Picshow
* Description    : ¶ÁBMPÍ¼Æ¬²¢ÏÔÊ¾
* Input          : x£¬y£ºÏÔÊ¾µØÖ·  bmp_name£ºÎÄ¼þÃû
* Output         : None
* Return         : None.
* Attention		 : None
*******************************************************************************/
void Picshow(int x,int y, const char *bmp_name)					 
{
  ...
  res = f_read(&file, newdata, 0x36, &br);//¶ÁÈ¡ÎÄ¼þÍ·ÐÅÏ¢
  if(newdata[0]==0x42 && newdata[1]==0x4D)//ÎÄ¼þ¸ñÊ½±ê¼ÇÎªBM
  {
  temp  = newdata[18]+(newdata[19]<<8)+(newdata[20]<<16)+(newdata[21]<<24);//Í¼Æ¬¿í¶ÈÐÅÏ¢
  temp1 = newdata[22]+(newdata[23]<<8)+(newdata[24]<<16)+(newdata[25]<<24);//Í¼Æ¬¸ß¶ÈÐÅÏ¢
  temp2 = newdata[28]+(newdata[29]<<8);																						//Í¼Æ¬Î»Éî¶ÈÐÅÏ¢
		//24Î»Éî¶ÈÍ¼Æ¬
		if(temp2==24)
		{
			LCD_SetCursor(0,0); 	
			...
			while(1)
			{
				res = f_read(&file, newdata, 3*1024, &br);//¶ÁÈ¡3¸öÊý¾Ý
				if(res||br==0)//´íÎóÌø³ö
				break;
				for(i=0;i<br/3;i++)
				{	/***************»­µãº¯Êý**************/					         
					/* Write 16-bit Reg */
					LCD_RAM = ((newdata[i*3+2]*32/256)<<11 | (newdata[i*3+1]*64/256)<<5 |(newdata[i*3]*32/256));
				}
			}
		}
		//32Î»Éî¶ÈÍ¼Æ¬
		...
		{	
			...
			while(1)
			{
				res = f_read(&file, newdata, 4*1024, &br);//¶ÁÈ¡3¸öÊý¾Ý
				if(res||br==0)   
				break;
				for(i=0;i<br/4;i++)
				{/***************»­µãº¯Êý**************/
					LCD_RAM = (((newdata[i*4+2]>>3)<<11 |(newdata[i*4+1]>>2)<<5 |(newdata[i*4]>>3)));
				}
			}
		}	
		//16Î»Éî¶ÈÍ¼Æ¬
		if(temp2==16)
		{	
			res = f_read(&file, newdata, 0x10, &br);//¶ÁÈ¡ÎÄ¼þÍ·ÐÅÏ¢
			LCD_SetCursor(0,0); 	
			LCD_WriteIndex(0x0022);
			while(1)
			{
				res = f_read(&file, newdata, 2*1024, &br);//¶ÁÈ¡3¸öÊý¾Ý
				if(res||br==0)   
				break;
				for(i=0;i<br/2;i++)
				{/***************»­µãº¯Êý**************/
					LCD_RAM = ((newdata[i*2+1]<<8 |(newdata[i*2])));
				}
			}
		}												
  }
...
{
  int i=0;
  int a=0,b=0;//aÎªºá×ø±ê£¬bÎª×Ý×ø±ê
  FIL file;//ÎÄ¼þ½á¹¹Ìå¶¨Òå
  UINT br;
  FRESULT res;
  uint8_t newtable[2];
  uint8_t table[2]={0,0};				 
  res = f_open(&file,txt_name, FA_OPEN_EXISTING | FA_READ);//´ò¿ªÎÄ¼þ        
  if(res!=FR_OK)//´ò¿ªÊ§°Ü´®¿Ú·¢ËÍÊ§°ÜÐÅÏ¢£¬¿ÉÊ¡ÂÔ
  printf("error");        
  while(1)	
  {
		res = f_read(&file, newdata,1024, &br);//¶ÁÈ¡1024¸öÊý¾Ý
		if(res || br == 0)//Ê§°Ü£¬ÍË³ö
		break;  
		kk:	while(i<br)
		{
			while(newdata[i]==13&&newdata[i+1]==10)	  //ÅÐ¶Ï»Ø³µ·û
			{
				b=b+16;//×Ý×ø±ê»»ÐÐ
				a=0;//ºá×ø±ê¹é0
				i=i+2;//Ìø¹ý»Ø³µ·û2¸ö
			}
			newtable[0]=newdata[i];//Ã¿Á½×Ö½Ú»º´æ
			newtable[1]=newdata[i+1];
			if(newdata[i]>128)//ÖÐÎÄÏÔÊ¾
			{
					if(i==1023)//³öÏÖ°ë¸ö×Ö½Ú
					{
					table[0]=newdata[i];//ÉÈÇøÄ©Î²°ë×Ö½Ú´æ´¢
					break;	
				    }
					if(!table[0])
					{
						PutChinese(a, b, newtable,White ,Black);//Õý³£ÏÔÊ¾
						i=i+2;
						a=a+16;																						 //ºá×ø±ê¼Ó16
					}	
						else//°ë×Ö½Ú´¦Àí
						{	
							table[1]=newdata[i];				 //ÁíÍâ°ë×Ö½Ú
							newtable[0]=table[0];
							newtable[1]=table[1];
							PutChinese(a, b, newtable, White,Black);
							i++;
							a=a+16;
							table[0]=0;		
						}
					if(a>225)									 //ºá×ø±ê³¬³ö·¶Î§£¬»»ÐÐ£¬225+16=241>240
					{
						a=0;
						b=b+16;
					}
				...
				{
					GUI_Text(a, b, newtable,White,Black);
					i=i+1;
					a=a+8;
					if(a>233)									//ºá×ø±ê³¬³ö·¶Î§£¬»»ÐÐ£¬233+8=241>240
					{
					a=0;
					b=b+16;
					}
				}
			while(b>305)//×Ý×ø±ê³¬³ö·¶Î§£¬»»Ò³
			{
			//	if(down==0)//ÅÐ¶Ï"ÏÂ"°´¼ü
			//	{
			//		b=0;
			...
			//		goto 	kk;//¼ÌÐøÏÔÊ¾
			//	}
			//	if(back==0)//ÅÐ¶Ï"·µ»Ø"¼ü
		        goto ref;
			}
		}
		i=0;//1024¸öÊý¾ÝÏÔÊ¾Íê£¬iÇåã
  }		   
f_close(&file);
ref: return;
}
